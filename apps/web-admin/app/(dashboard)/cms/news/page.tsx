...
export default function CmsNewsPage() {
  ...
  useEffect(() => {
    MockApi.getCmsNews().then((res) => {
      setData(res.filter((r) => r.type === "news"));
      setLoading(false);
    });
  }, []);
  ...
  return (
    <div className="max-w-[1200px] mx-auto flex flex-col gap-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-primary)] tracking-tight">Yangiliklar</h1>
          <p className="text-sm text-[var(--text-muted)] mt-1">Saytdagi yangiliklar va maqolalar</p>
        </div>
        <Button size="sm" icon={<Plus size={14} />}>Yangilik qo'shish</Button>
      </div>
      <DataTable columns={columns} data={data} keyField="id" emptyMessage="Yangiliklar topilmadi" />
    </div>
  );
}
