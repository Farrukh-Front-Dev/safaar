...
export default function CmsPagesPage() {
  ...
  const columns: Column<CmsArticle>[] = [
    ...
    { key: "publishedAt", label: "Oxirgi tahrir", render: (row) => <span className="text-sm">{row.publishedAt ? formatDate(row.publishedAt) : "—"}</span> },
    {
      key: "actions",
      label: "",
      render: () => (
  ...
  return (
    <div className="max-w-[1200px] mx-auto flex flex-col gap-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-primary)] tracking-tight">Sayt Sahifalari</h1>
          <p className="text-sm text-[var(--text-muted)] mt-1">Haqimizda, FAQ kabi statik sahifalar matnlari</p>
        </div>
        <Button size="sm" icon={<Plus size={14} />}>Sahifa qo'shish</Button>
      </div>
      <DataTable columns={columns} data={data} keyField="id" emptyMessage="Sahifalar topilmadi" />
    </div>
  );
}
