...
class Title {
  ...
  createMesh() {
    ...
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textHeightScaled * 0.6 - 0.05;
...
class App {
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    ...
    // Click detection: boshlash va tugash orasida 5px dan kam harakat = click
    const endX = 'touches' in e
      ? (e as TouchEvent).changedTouches[0]?.clientX ?? this.start
      : (e as MouseEvent).clientX;
    const moved = Math.abs(endX - this.start);

    if (moved < 5 && this.onItemClick) {
      // Markazga eng yaqin item'ni topamiz
      const closest = this.findClosestItemIndex();
      if (closest !== -1) this.onItemClick(closest % (this.mediasImages.length / 2));
    }
  ...
  /**
   * Viewport markaziga (x=0) eng yaqin media item'ni topadi.
   * Medialar duplicate bo'lgani uchun original length'ga mod olamiz.
   */
  findClosestItemIndex(): number {
  ...
  update() {
    // Auto-scroll: foydalanuvchi tegmayotganda va hover qilmaganda sekin o'zi siljiydi
    if (!this.isDown && !this.isHovered) {
      this.scroll.target += this.scrollSpeed * 0.015;
    }
  ...
  addEventListeners() {
    ...
    // Hover'da auto-scroll to'xtaydi
