  ...
  return (
    <aside aria-label={dict.filters.title} className="lg:sticky lg:top-20 lg:h-fit">
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
        className="mb-3 flex w-full items-center justify-between rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium lg:hidden"
      >
        {dict.filters.toggle}
      ...
      <div
        className={cn(
          "flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm",
          open ? "flex" : "hidden",
          "lg:flex",
        )}
      >
