 ...
 *
 * Bu taxmin demo data va `app` mock qiymatlari asosida aniqlandi. Agar backend
 * dev boshqacha deb tasdiqlasa — faqat shu fayldagi `TIYIN_PER_SUM`ni o'zgartirsa
 * bo'ladi, qolgan kod tegmaydi.
 */
...
export function formatSum(sum: number): string {
  return `${Math.round(sum).toLocaleString("uz-UZ")} so'm`;
}
